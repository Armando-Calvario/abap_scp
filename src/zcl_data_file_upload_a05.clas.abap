CLASS zcl_data_file_upload_a05 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_data_file_upload_a05 IMPLEMENTATION.
...
* extract the filename and fileext from this metadataas well as the raw file data.
        ...
        LOOP AT content REFERENCE INTO content_item.
          " PUT it all BACK together again humpdy dumpdy....
...
* Check file extension is valid, only json today.
        IF fileext <> `json`.
          response->set_status( i_code =
          if_web_http_status=>bad_request
          i_reason = `File typenot supported` ).
